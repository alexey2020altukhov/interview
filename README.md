...
- Брокеры сообщений
- KeyCloak
- Feign Client, retry (пример паттерна Circuit Breaker)
- Создание сущности под VIEW
- Паттерны проектирования МСА  (дописать)
- JDBC, Hibernate, JPA
- Lazy и Eager инициализация
- Мертвые строки и вакуум
- Пул соединений, HikariCP
- Pageable
- VIEW, их виды и отличия, как сделать в коде?
- Индексы, виды, какой используется по умолчанию (дописать)
- Партицирование таблиц
- Как выбрать оптимальное количество партиций в Kafka (по количеству consumers)?
- В каких случаях использовать Kafka, в каких REST? Kafka - асинхрон, REST - синхрон
- Как можем амортизировать нагрузку на систему с помощью Kafka?
- Форматы сообщений в Kafka
- Можем ли сделать, чтобы GET работал как POST, и наоборот? Для чего это делается?
...
## Теорема CAP
В распределенной системе можно обеспечить только два свойства из трех: согласованность, доступность и устойчивость к разделению. Теорема помогает понять, как конкретная распределенная система будет работать и чего от нее ожидать.
...
- Сообщения публикуются в топики, а потребители подписываются на этот топик и читают из него 
...
``wait()`` - приостанавливает выполнение текущего потока, пока другой поток не вызовет notify() или notifyAll().

``wait(long timeout)`` - приостанавливает выполнение текущего потока на определенное время в миллисекундах.

``wait(long timeout, int nanos)`` - приостанавливает выполнение текущего потока на определенное время с точностью до наносекунд.
...
По умолчанию метод equals() сравнивает два объекта по ссылкам.
...
- B-tree (сбалансированное дерево) — это самый распространенный тип индекса в PostgreSQL. Поддерживает все стандартные операции сравнения (>, <, >=, <=, =, <>) и может использоваться с большинством типов данных. B-tree индексы могут быть использованы для сортировки, ограничений уникальности и поиска по диапазону значений.
...
**Nodes (Ноды, узлы)** - виртуальные или физические серверы, на которых работает Kubernetes. Они бывают двух типов:
- Worker Node – сервер на котором запускаются и работают контейнеры.
- Master Node – сервер который управляет Worker Nodes.

**Кластер (Cluster)** — это группа узлов, объединенных для работы вместе. 
**Pod** — это минимальная единица работы в Kubernetes, состоящая из одного или нескольких контейнеров, которые размещены вместе на одном узле. 
...
Master Nodes состоит из компонентов:
- kube-apiserver - сервер API, который обеспечивает работу API кластера, обрабатывает REST-операции и предоставляет интерфейс, через который все остальные компоненты взаимодействуют между собой.
- kube-controller-manager - запускает работу контроллеров.
- kube-scheduler - планировщик, который определяет на каких узлах разворачивать pods.
...
**Многопоточность** — это одновременное выполнение нескольких независимых потоков в одном процессе.

**Процесс** — это совокупность кода и данных, разделяющих общее виртуальное адресное пространство. Процессы изолированы друг от друга, поэтому прямой доступ к памяти чужого процесса невозможен. 

**Поток** - более легковесная единица выполнения, существующая внутри процесса. В отличие от процессов, потоки внутри одного процесса совместно используют его ресурсы.
...
**Что можно обозначить как synchronized?**
...
Монитор - это объект, используемый в качестве взаимоисключающей блокировки. Когда метод или блок объявлен как synchronized, он получает монитор объекта, на котором вызывается этот метод или блок. Монитор позволяет только одному потоку за раз входить в блок синхронизации.
